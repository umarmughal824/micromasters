...
describe('Course Progress', () => {
  const course = findAndCloneCourse(course => (
    course !== null && course !== undefined && course.runs.length > 0
  ));
  course.runs[0].current_grade = 92;

  const renderCourseProgress = () => shallow(
    <Progress courseRun={course.runs[0]} className="classname" />
  );

  it('should the current progress for a course', () => {
    let progress = renderCourseProgress();
  ...
  it('shows a progress bar', () => {
    ...
    assert.deepEqual(
      progress.find('.course-progress-bar').props().style,
      { width: '92%' }
    );
  });
});
