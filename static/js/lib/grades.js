...
const findLargestGrade = key => R.compose(
  R.prop(key),
  R.reduce(
    R.maxBy(R.prop(key)), { [key]: 0 }
  )
);
...
export const calculateFinalGrade = R.converge(
  S.lift2(R.add),
  [
    R.compose(
      S.map(R.multiply(COURSE_GRADE_WEIGHT)),
      getLargestEdXGrade
    ),
    R.compose(
      S.map(R.multiply(EXAM_GRADE_WEIGHT)),
      getLargestExamGrade
    )
  ]
);
