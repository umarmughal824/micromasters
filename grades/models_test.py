...
class MicromastersCertificateTests(MockedESTestCase):
    """Tests for MicromastersCertificate"""

    def test_autogenerated_hash(self):
        """Test that MicromastersCertificate auto-generates a hash when none is provided"""
        user = UserFactory.create()
        ...
        mm_certificate = MicromastersCourseCertificate.objects.create(final_grade=final_grade)
        assert len(mm_certificate.hash) == 32
        assert mm_certificate.hash == generate_md5('{}|{}'.format(user.id, course_run.id).encode('utf-8'))
