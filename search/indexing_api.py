...
def serialize_public_enrolled_user(serialized_enrolled_user):
    ...
    return {
        ...
        '_type': PUBLIC_USER_DOC_TYPE,  # override the default of USER_DOC_TYPE
...
def _get_program_enrolled_users_documents(program_enrollments):
    """
    Generator for documents to be indexed given a set of program enrollments
    Args:
    ...
    for program_enrollment in program_enrollments:
        ...
        if document is None:
            continue
        yield document  # uses the default doctype
...
def index_program_enrolled_users(program_enrollments, indices=None, chunk_size=100):
    ...
    for index in indices:
        count = _index_chunks(
            _get_program_enrolled_users_documents(program_enrollments),
            USER_DOC_TYPE,
            index,
...
def remove_program_enrolled_user(program_enrollment_id, indices=None):
    ...
    for index in indices:
        ...
        _delete_item(program_enrollment_id, PUBLIC_USER_DOC_TYPE, index)
...
def public_program_enrolled_user_mapping():
    """
    Builds the raw mapping data for the program-enrolled user doc type
    """
    mapping = Mapping(PUBLIC_USER_DOC_TYPE)
    mapping.field("id", "long")
    mapping.field("user_id", "long")
    mapping.field("profile", "object", properties={
        'account_privacy': NOT_ANALYZED_STRING_TYPE,
...
def _create_mappings(index_name):
    ...
    # private (staff search) mapping of enrolled users
    ...
    # public mapping of enrolled users
    if conn.indices.exists_type(index=index_name, doc_type=PUBLIC_USER_DOC_TYPE):
        conn.indices.delete_mapping(index=index_name, doc_type=PUBLIC_USER_DOC_TYPE)
    mapping = public_program_enrolled_user_mapping()
    mapping.save(index_name)
